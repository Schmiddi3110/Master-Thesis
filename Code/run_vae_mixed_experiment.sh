...
for dataset in "PenDigits" "WalkingSittingStanding" "SpokenArabicDigits"; do
    for real_percentage in "${real_percentages[@]}"; do
        ...
        python3 ./Experiments/mixed_generated/run_vae_mixed_experiment.py --dataset $dataset --real_percentage $real_percentage
    done
    python3 ./misc/plots/mixed_generated_plot.py --dataset $dataset --real_percentage "${real_percentages[@]}"
done
