...
defense_dir = f"TimeSeriesModels/{dataset}/Results/Noise_VAE_Defense/{defense_type}"
...
for pairing in model_pairings:
    ...
    for ax, defense in zip(axes, defenses):
        ...
        for name, group in subset.groupby("Method Name"):
            group["Num Samples"] = group["Real Samples"] + group["Generated Samples"] + group["Noised Samples"]

            aggregated = group.groupby("Num Samples")["Stolen Model Accuracy"].mean().reset_index()
            line, = ax.plot(aggregated["Num Samples"], aggregated["Stolen Model Accuracy"], label=name)
    ...
    plt.suptitle(f"{original_model} → {extracted_model} Across {defense_type} Defenses", fontsize=16, y=0.98)
    plt.tight_layout(rect=[0, 0.1, 1, 0.95])

    os.makedirs(f"TimeSeriesModels/{dataset}/Results/Noise_VAE_Defense/{defense_type}/plots", exist_ok=True)
    plt.savefig(f"TimeSeriesModels/{dataset}/Results/Noise_VAE_Defense/{defense_type}/plots/{original_model}_to_{extracted_model}_defended.jpg", bbox_inches="tight")
    plt.close(fig)
