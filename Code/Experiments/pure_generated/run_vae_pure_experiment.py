...
from Experiments.pure_generated.pure_generate_attack import run_pure_generated_data_experiment
...
gen_results.extend(run_pure_generated_data_experiment(real_percentage, N, conv_architectures_catalogue, X_test0, y_test0, batch_size, num_epochs, learning_rate, latent_dim, device, dataset))
df = pd.DataFrame(gen_results, columns=['Percentage Generated Data', 'Iteration', 'Model Pairing', 'Method Name', 'Num Samples', 'Accuracy', 'Original Model Accuracy'])

os.makedirs(f'TimeSeriesModels/{dataset}/Results/PureVAE/plots', exist_ok=True)        
df.to_csv(f"TimeSeriesModels/{dataset}/Results/PureVAE/VAE_results_{real_percentage}.csv", index=False)
