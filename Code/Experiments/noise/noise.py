...
def run_noise_experiment(noise_level, N, catalogue, X_test0, y_test0, batch_size, num_epochs, learning_rate):
    num_classes = len(np.unique(y_test0))
    conv_model_results = []
    for iteration in range(N):
            print(f"Iteration {iteration + 1}/{N}")
            
            for model_pairing in catalogue:
                print(f"Running attacks for: {model_pairing}")
                
                
                model, orginal_model_accuracy = catalogue[model_pairing][0]
                criterion = nn.CrossEntropyLoss()
                if catalogue[model_pairing][2] == "aeon":
                ...
                elif catalogue[model_pairing][2] == "torch":
                    ...
                    raise ValueError(f"Unrecognized Classifier: {catalogue[model_pairing][2]}")
                ...
                for percentage in percentages:
                    ...
                    x_steal = add_noise(x_steal, noise_level=noise_level)            
                    ...
                    for name, attack in extraction_attacks.items():
                        model_stolen = catalogue[model_pairing][1]
                        criterion = nn.CrossEntropyLoss()
                        optimizer = optim.Adam(model_stolen.parameters(), lr=learning_rate)

                        classifier_stolen = PyTorchClassifier(model=model_stolen, loss=criterion, optimizer=optimizer, input_shape=(batch_size, X_test0.shape[1], X_test0.shape[2]), nb_classes=num_classes)
                        ...
                        del model_stolen, classifier_stolen, optimizer
                        torch.cuda.empty_cache()

                        conv_model_results.append((noise_level, iteration + 1, model_pairing, name, len_steal, acc, orginal_model_accuracy))
