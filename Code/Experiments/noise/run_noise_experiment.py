...
from misc.utils import normalize_data, pad_time_series, create_folders
...
from Experiments.noise.noise import run_noise_experiment
...
noise_results.extend(run_noise_experiment(noise_level, N, conv_architectures_catalogue, X_test0, y_test0, batch_size, num_epochs, learning_rate))

torch.cuda.empty_cache()

df = pd.DataFrame(noise_results, columns=['Noise Level', 'Iteration', 'Model Pairing', 'Method Name', 'Stealing Dataset Size', 'Accuracy', 'Original Model Accuracy'])
df.to_csv(f"TimeSeriesModels/{dataset}/Results/Noise/noise_results_{noise_level}.csv", index=False)
